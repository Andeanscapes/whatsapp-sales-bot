...
2. Create `.env` from `.env.example` with real local/prod values.
3. Start app:
...
The SQLite database stays in the `bot-data` Docker volume.
...
4. Create `.env` with production values. Do not commit it.
