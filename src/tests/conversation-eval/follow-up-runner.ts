...
export async function runFollowUpScenario(ctx: RunContext, scenario: Scenario): Promise<TurnRecord[]> {
  ...
  const previousSendStartHour = env.FOLLOW_UP_SEND_START_HOUR;
  const previousSendEndHour = env.FOLLOW_UP_SEND_END_HOUR;
  const originalFetch = globalThis.fetch;
  const turns: TurnRecord[] = [];

  ctx.repos.conversation.upsert(ctx.customerPhone, {
    language: scenario.lang,
    sales_phase: scenario.followUpSeed?.phase,
    lead_score: scenario.followUpSeed?.score,
    collected_name: scenario.followUpSeed?.qualification?.name,
    collected_people: scenario.followUpSeed?.qualification?.people,
    collected_date: scenario.followUpSeed?.qualification?.date,
  ...
  try {
    ...
    env.FOLLOW_UP_SEND_START_HOUR = previousSendStartHour;
    env.FOLLOW_UP_SEND_END_HOUR = previousSendEndHour;
  }
